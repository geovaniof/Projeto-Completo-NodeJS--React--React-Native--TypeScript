...
useEffect(() => {
  const tarefasStorage = localStorage.getItem("@tarefa"); //Recupera o item do localStorage
  if (tarefasStorage) {
    setTarefas(JSON.parse(tarefasStorage)) //Converte a string de volta para um array e atualiza o estado
  }
}, []) ; //Array de dependências vazio -> Executa apenas uma vez quando o componente monta

/*
  Cada vez que monta o componente, o useEffect é executado
*/
useEffect(() => {
  localStorage.setItem("@tarefa", JSON.stringify(tarefas)); //Salva o array de tarefas no localStorage

}, [tarefas]); //Toda a vez que o array de tarefas for alterado, o useEffect será executado
